...
DESCRIBE_FILE = $(obj)include/describe_autogenerated.h
...
version:
		...
		echo "\"" >> $(VERSION_FILE)	\
		;								\
		echo -n $(shell $(CONFIG_SHELL) $(TOPDIR)/tools/chklocaldescribe \
				 $(TOPDIR) $(DESCRIBE_FILE)); \
...
ifeq ($(OVER_CLOCK),yes)
	@echo "#define CONFIG_YW_OVER_CLOCK" >>include/config.h
endif
ifeq ($(YW_XPROG),yes)
	@echo "#define CONFIG_YW_XPROG" >>include/config.h
