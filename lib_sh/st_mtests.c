...
int do_st_memory_test(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	printf("do_st_memory_test: Start: 0x%08x - Length: 0x%08x\n",
	       start, length);
	ret = memory_test(start, length);
	return(ret);
}

#define CONTROL_BITS(x) ((unsigned long)x & 0xFFF80000)
#define ADDRESS_BITS(x) ((unsigned long)x & 0x00F7FFFF)

#define ALL_ZERO 0x00000000
#define ALL_FIVE 0x55555555
#define ALL_AAAA 0xAAAAAAAA
