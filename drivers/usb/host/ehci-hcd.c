...
#warning CONFIG_SYS_MALLOC_LEN may be too small for EHCI
	 ...
	 */
	qh->qh_link = cpu_to_hc32((uint32_t)PHYSICAL_ADDR(&ctrl->qh_list) | QH_LINK_TYPE_QH);
	...
	ctrl->qh_list.qh_link = cpu_to_hc32((uint32_t)PHYSICAL_ADDR(qh) | QH_LINK_TYPE_QH);
	...
	ehci_writel(&ctrl->hcor->or_asynclistaddr, (uint32_t)PHYSICAL_ADDR(&ctrl->qh_list));
