...
static int ep_link(ohci_t *ohci, ed_t *edi)
{
	...
	case PIPE_CONTROL:
		...
		if (ohci->ed_controltail == NULL)
			ohci_writel(PHYSICAL_ADDR(ed), &ohci->regs->ed_controlhead);
		else
			ohci->ed_controltail->hwNextED =
					   PHYSICAL_ADDR(m32_swap((unsigned long)ed));
	...
	case PIPE_BULK:
		...
		if (ohci->ed_bulktail == NULL)
			ohci_writel(PHYSICAL_ADDR(ed), &ohci->regs->ed_bulkhead);
		else
...
pkt_print(NULL, dev, pipe, buffer, transfer_len,
	...
	*/
	...
	case RH_GET_STATUS:
		*(__u16_any *) data_buf = cpu_to_le16(1);
		OK(2);
	case RH_GET_STATUS | RH_INTERFACE:
		*(__u16_any *) data_buf = cpu_to_le16(0);
		OK(2);
...
static int hc_start(ohci_t *ohci)
{
	...
	ohci_writel(PHYSICAL_ADDR((__u32)ohci->hcca),
