...
static void stxh301_clocks(void)
{
	/*
	 ...
	 */
#if (CONFIG_SYS_STM_ASC_BASE==STXH301_SBC_ASC0_BASE) || (CONFIG_SYS_STM_ASC_BASE==STXH301_SBC_ASC1_BASE)
 ...
 */
extern void stxh301_pioalt_select(int port, const int pin, const int alt)
{
	...
	case 0 ... 5:		/* in "SBC Bank" */
		sysconfReg = (unsigned long*)STXH301_SYSCFG(0);
		sysconfReg += port;
		break;
	case 10 ... 20:		/* in "FRONT Bank" */
		sysconfReg = (unsigned long*)STXH301_SYSCFG(1000);
		sysconfReg += port - 10;
		break;
	case 30 ... 35:		/* in "REAR Bank" */
		sysconfReg = (unsigned long*)STXH301_SYSCFG(2000);
		sysconfReg += port - 30;
		break;
	case 40 ... 42:		/* in "FLASH Bank" */
		sysconfReg = (unsigned long*)STXH301_SYSCFG(3000);
		sysconfReg += port - 40;
		break;
	default:
		BUG();
...
extern void stxh301_pioalt_pad(int port, const int pin,
		const enum stm_pad_gpio_direction direction)
{
	...
	{
	case 0 ... 5:		/* in "SBC Bank" */
		sysconfReg = (unsigned long*)STXH301_SYSCFG(40);
		sysconfReg += port / 4;
		break;
	case 10 ... 20:		/* in "FRONT Bank" */
		port -= 10;
		sysconfReg = (unsigned long*)STXH301_SYSCFG(1040);
		sysconfReg += port / 4;
		break;
	case 30 ... 35:		/* in "REAR Bank" */
		port -= 30;
		sysconfReg = (unsigned long*)STXH301_SYSCFG(2040);
		sysconfReg += port / 4;
		break;
	case 40 ... 42:		/* in "FLASH Bank" */
		port -= 40;
		sysconfReg = (unsigned long*)STXH301_SYSCFG(3040);
		sysconfReg += port / 4;
...
static void stxh301_configure_pios(
	...
	const size_t num_pads)
{
	...
	if (debug_pad_configs)
		...
		stxh301_pioalt_select(portno, pinno, pad->pio.alt);
		stxh301_pioalt_pad(portno, pinno, pad->direction);
		if (pad->retime)
			stxh301_pioalt_retime(portno, pinno, pad->retime, pad->direction);
	}
}
...
extern void stxh301_pioalt_retime(int port, const int pin,
		...
		const enum stm_pad_gpio_direction direction)
{
	...
	{
	case 0 ... 5:		/* in "SBC Bank" */
		sysconfReg = (unsigned long*)STXH301_SYSCFG(100);
		sysconfReg += (port-0) * 8;
		break;
	case 10 ... 20:		/* in "FRONT Bank" */
		sysconfReg = (unsigned long*)STXH301_SYSCFG(1100);
		sysconfReg += (port-10) * 8;
		break;
	case 30 ... 35:		/* in "REAR Bank" */
		sysconfReg = (unsigned long*)STXH301_SYSCFG(2100);
...
static struct stm_pad_pin stxh301_ethernet_mii_pad_configs[] = {
...
static struct stm_pad_sysconf stxh301_ethernet_mii_pad_sysconfs[] = {
			/* EN_GMAC1 */
...
static struct stm_pad_pin stxh301_ethernet_rgmii_pad_configs[] = {
...
static struct stm_pad_sysconf stxh301_ethernet_rgmii_pad_sysconfs[] = {
			/* SBC_RESET_PER */
...
#if 0 //imran cmntd --did not find any rmii usage
...
static void stxh301_ethernet_bus_setup(void)
{
	...
	const struct stxh301_ethernet_config * const config)
{
...
#if 0 //imran cmntd -- no use of rmii so far
	case stxh301_ethernet_mode_mii:
		pad_config = stxh301_ethernet_mii_pad_configs;
		num_pads = ARRAY_SIZE(stxh301_ethernet_mii_pad_configs);
		sys_configs = stxh301_ethernet_mii_pad_sysconfs;
		num_sys = ARRAY_SIZE(stxh301_ethernet_mii_pad_sysconfs);
...
#endif
	case stxh301_ethernet_mode_rgmii:
		 ...
		 */
		pad_config = stxh301_ethernet_rgmii_pad_configs;
		num_pads = ARRAY_SIZE(stxh301_ethernet_rgmii_pad_configs);
		sys_configs = stxh301_ethernet_rgmii_pad_sysconfs;
		num_sys = ARRAY_SIZE(stxh301_ethernet_rgmii_pad_sysconfs);
		phy_clock = stm_gmac_find_phy_clock(pad_config, num_pads);
		stm_pad_set_pio_out(phy_clock, 4);
		break;
#if 0 //imran --not using these modes in enum
	...
	default:
		...
		/* now configure all the PIOs */
	stxh301_configure_pios(pad_config, num_pads);
...
static void stxh301_init_dwc3(void)
{
	unsigned long * sysconfReg ;
	unsigned long sysconf ;
	...
	/* Update control*/
	sysconfReg = (unsigned long*)STXH301_SYSCFG(5068);
	sysconf = readl(sysconfReg);
	...
	/* */
	sysconfReg = (unsigned long*)STXH301_SYSCFG(5061);
	sysconf = readl(sysconfReg);
	/*PICOPHY_REFCLKSEL*/
	SET_SYSCONF_BIT(sysconf,0 , 0);
	...
	sysconfReg = (unsigned long*)STXH301_SYSCFG(5064);
	sysconf = readl(sysconfReg);
	SET_SYSCONF_BIT(sysconf,0 , 0);
	SET_SYSCONF_BIT(sysconf,0 , 1);
	SET_SYSCONF_BIT(sysconf,1 , 2);
...
#if 0
	sysconfReg = (unsigned long*)STXH301_SYSCFG(5001);
	sysconf = readl(sysconfReg);
	SET_SYSCONF_BIT(sysconf, 1, 6);
	writel(sysconf, sysconfReg);
...
{
	stxh301_pioalt_select(35, 4, 0);
	stxh301_pioalt_select(35, 5, 0);
	stxh301_pioalt_select(35, 6, 0);

	stxh301_pioalt_pad(35, 4, stm_pad_direction_output);
	stxh301_pioalt_pad(35, 5, stm_pad_direction_input);
	stxh301_pioalt_pad(35, 6, stm_pad_direction_output);
}

static void configure_usb_pio(void)
{
	stxh301_pioalt_select(35, 4, 1);
	stxh301_pioalt_select(35, 5, 1);
	stxh301_pioalt_select(35, 6, 1);

	stxh301_pioalt_pad(35, 4, stm_pad_direction_input);
	stxh301_pioalt_pad(35, 5, stm_pad_direction_output);
	stxh301_pioalt_pad(35, 6, stm_pad_direction_input);
}
...
extern int arch_cpu_init(void)
{
	...
	gd->arch.stm_devid = *STXH301_SYSCONF_DEVICEID;
...
extern void stxh301_configure_sata(void)
{
#if 0	/* QQQ - TO IMPLEMENT */
...
extern void stxh301_configure_i2c(void)
{
	...
	stxh301_pioalt_select(scl_port, scl_pin, 0);			/* I2C_SCL */
	stxh301_pioalt_select(sda_port, sda_pin, 0);			/* I2C_SDA */
...
extern void stxh301_configure_spi(void)
{
	 ...
	 */
	STXH301_PIOALT_SELECT(SPI_nCS,  1);	/* SPI_nCS */
	STXH301_PIOALT_SELECT(SPI_CLK,  1);	/* SPI_CLK */
	STXH301_PIOALT_SELECT(SPI_MOSI, 1);	/* SPI_D0 */
	STXH301_PIOALT_SELECT(SPI_MISO, 1);	/* SPI_D1 */
		/* following only really needed for x4 mode */
	STXH301_PIOALT_SELECT(SPI_nWP,  1);	/* SPI_D2 */
	STXH301_PIOALT_SELECT(SPI_HOLD, 1);	/* SPI_D3 */
	...
	STXH301_PIOALT_SELECT(SPI_MISO, 0);	/* SPI_MISO */
	STXH301_PIOALT_SELECT(SPI_MOSI, 0);	/* SPI_MOSI */
	STXH301_PIOALT_SELECT(SPI_nCS,  0);	/* SPI_nCS */
	STXH301_PIOALT_SELECT(SPI_CLK,  0);	/* SPI_CLK */
...
{
	STPIO_301_PIN2(SPI_CLK, val ? 1 : 0);
}
...
static const struct stm_pad_pin stxh301_mmc0_pad_configs[] = {
...
static const struct stm_pad_pin stxh301_mmc1_pad_configs[] = {
...
static void stxh301_enable_mmc(const int port, const u32 regbase)
{
	if (port != 0)
	{
...
/**
 * stxh301_mmc_core_config: configure the Arasan HC
 * @ioaddr: base address
 ...
 * This happens on some SoCs, i.e. StiH301, where the MMC0 inside the flashSS
 ...
 */
static void stxh301_mmc_core_config(const int port, const u32 regbase)
{
#ifdef MMC_CORE_DEBUG
...
static void stxh301_configure_mmc(const int port, const u32 regbase)
{
	...
	if (port==0)
	{
		stxh301_enable_mmc(port, regbase);
		stxh301_mmc_core_config(port, regbase);
		pad_config = stxh301_mmc0_pad_configs;
		num_pads = ARRAY_SIZE(stxh301_mmc0_pad_configs);
	}
	else
#endif	/* CONFIG_STM_SDHCI_0 */
#if defined (CONFIG_STM_SDHCI_1)
	if (port==1)
	{
		stxh301_enable_mmc(port, regbase);
		stxh301_mmc_core_config(port, regbase);
		pad_config = stxh301_mmc1_pad_configs;
		num_pads = ARRAY_SIZE(stxh301_mmc1_pad_configs);
	}
	else
#endif	/* CONFIG_STM_SDHCI_1 */
	{
...
#if defined(CONFIG_STM_SDHCI_0)
	stxh301_configure_mmc(0, CONFIG_SYS_MMC0_BASE);
	ret |= stm_sdhci_init(0, CONFIG_SYS_MMC0_BASE);
#endif	/* CONFIG_STM_SDHCI_0 */

#if defined (CONFIG_STM_SDHCI_1)
	stxh301_configure_mmc(1, CONFIG_SYS_MMC1_BASE);
...
extern void stxh301_configure_nand(void)
{
	 ...
	 */
	stxh301_pioalt_select(40, 4, 3);	/* NAND_CSn3 */
	stxh301_pioalt_select(40, 5, 3);	/* NAND_CSn2 */
	stxh301_pioalt_select(40, 6, 3);	/* NAND_CSn1 */
	stxh301_pioalt_select(40, 7, 3);	/* NAND_CSn0 */

	stxh301_pioalt_select(41, 0, 3);	/* NAND_DQ0 */
	stxh301_pioalt_select(41, 1, 3);	/* NAND_DQ1 */
	stxh301_pioalt_select(41, 2, 3);	/* NAND_DQ2 */
	stxh301_pioalt_select(41, 3, 3);	/* NAND_DQ3 */
	stxh301_pioalt_select(41, 4, 3);	/* NAND_DQ4 */
	stxh301_pioalt_select(41, 5, 3);	/* NAND_DQ5 */
	stxh301_pioalt_select(41, 6, 3);	/* NAND_DQ6 */
	stxh301_pioalt_select(41, 7, 3);	/* NAND_DQ7 */

	stxh301_pioalt_select(42, 0, 3);	/* NAND_WEn */
	stxh301_pioalt_select(42, 2, 3);	/* NAND_ALE */
	stxh301_pioalt_select(42, 3, 3);	/* NAND_CLE */
	stxh301_pioalt_select(42, 4, 3);	/* NAND_RnB */
	stxh301_pioalt_select(42, 5, 3);	/* NAND_REn */
}
