				...
				cfiword_t cword)
{
	...
	if (use_flash_status_poll(info))
		retcode = flash_status_poll(info, &cword, dstaddr,
	...
	if (flash_detect_cfi (info, &qry)) {
		...
		for (i = 0; i < num_erase_regions; i++) {
			...
			debug ("erase_region_count = %d erase_region_size = %d\n",
				 ...
				 *	flash_info_t.start[CONFIG_SYS_MAX_FLASH_SECT];
				 *	flash_info_t.protect[CONFIG_SYS_MAX_FLASH_SECT];
				 */
			if (erase_region_count > CONFIG_SYS_MAX_FLASH_SECT) {
				printf ("Error: Number of Sectors (%d) > CONFIG_SYS_MAX_FLASH_SECT (%d)\n",
					erase_region_count,
